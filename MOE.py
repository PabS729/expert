...
class MoE(nn.Module):
    ...
    def noisy_top_k_gating(self, x, train, noise_epsilon=1e-2):
        ...
        #print(gates)
        # nd = torch.ones_like(gates).cuda()
        # nd[:int(gates.shape[0]/2)] = 0
        # sh = torch.hstack((nd, gates))[:64]
        # sp = torch.hstack((nd, gates))[64:]
        # print(sh)
        # print(sp)
        #print(gates.shape)
    ...
    def forward(self, x, train=True, loss_coef=3e-3):
        """Args:
        ...
        #print(torch.sum(gates[0] > 0.5), torch.sum(gates[1] > 0.5))
